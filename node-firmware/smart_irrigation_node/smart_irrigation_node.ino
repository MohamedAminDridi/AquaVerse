...
#include <esp_sleep.h>   // deep-sleep duty-cycle mode
#include <esp_system.h>  // esp_reset_reason() — tell deep-sleep wake from a real restart
#include <Wire.h>
...
// In sleep mode the node stays awake g_awakeSec each cycle (reporting +
// accepting commands), then deep-sleeps g_wakeIntervalSec, and repeats. State
// lives in RTC memory so it survives the reboot deep sleep performs on wake; a
// real power-on/reset clears it (handled in setup) so a restart disables sleep.
RTC_DATA_ATTR bool     g_sleepMode       = false;   // duty-cycle active?
RTC_DATA_ATTR uint32_t g_wakeIntervalSec = 3600;    // SLEEP duration — how long to deep-sleep
RTC_DATA_ATTR uint32_t g_awakeSec        = 9;       // AWAKE duration — how long to stay up each cycle
RTC_DATA_ATTR uint32_t g_bootCount       = 0;       // wakes since power-on
uint32_t g_wakeMillis = 0;       // millis() at this wake — when the awake window started
...
// ── Sleep duty-cycle timing (so the UI countdown tracks the real node) ──
// Every packet carries the node's own clock for the duty cycle:
//   slp = sleep mode on?  awk = awake window (s)  nap = sleep window (s)
//   up  = seconds elapsed since this wake started (millis - g_wakeMillis)
// The backend forwards these and the dashboard anchors its countdown to them,
// re-syncing on each packet instead of guessing from config alone.
static void addSleepInfo(JsonDocument& doc) {
  doc["slp"] = g_sleepMode ? 1 : 0;
  if (g_sleepMode) {
    doc["awk"] = g_awakeSec;
    doc["nap"] = g_wakeIntervalSec;
    doc["up"]  = (uint32_t)((millis() - g_wakeMillis) / 1000);
  }
}

// ── Compact telemetry schema (to fit the 255-byte LoRa limit) ──────
// Only "type" + "id" keep long names (the gateway routes on them). Everything
// else is squeezed to 1–2 char keys; bools are 0/1; the valve string is dropped
// (the backend infers open/closed from vp>0). The backend parser maps these back
// to full names and still accepts the old long keys for compatibility.
//   s  soil %        t  temp °C       h  humidity %     b  battery %
//   bv batt volts    bi batt mA       bm batt mAh       tm  min to full/empty
//   c  charging 0/1  q  seq           vp valve %        p   pump 0/1
//   slp/awk/nap/up   sleep duty-cycle (added by addSleepInfo when sleeping)
static void fillTelemetry(JsonDocument& doc, float temp, float hum) {
  doc["type"] = "telemetry";          // gateway routes on type + id — keep these
  doc["id"]   = DEVICE_ID;
  doc["s"]    = readSoilPct();
  if (!isnan(temp)) doc["t"] = round(temp * 10) / 10.0;
  if (!isnan(hum))  doc["h"] = round(hum  * 10) / 10.0;
  doc["b"]    = batterySoC();
  doc["bv"]   = round(readBatteryVoltage() * 100) / 100.0;
  doc["bi"]   = round(readBatteryCurrent_mA() * 10) / 10.0;
  doc["bm"]   = (int)g_batMah;
  doc["tm"]   = batteryTimeMin();
  doc["c"]    = batteryCharging() ? 1 : 0;
  doc["q"]    = seq++;
  doc["vp"]   = valvePercent;          // 0 ⇒ closed (backend infers valve state)
  doc["p"]    = pumpRunning ? 1 : 0;
  addSleepInfo(doc);
}
...
void sendStatus() {
  JsonDocument doc;
  fillTelemetry(doc, dht.readTemperature(), dht.readHumidity());
  sendPacket(doc);
}
...
// Power down the radio and enter timed deep sleep. Reboots on wake (setup runs).
void enterDeepSleep() {
  Serial.printf("[SLEEP] deep sleep for %u s (wake #%u)\n", g_wakeIntervalSec, g_bootCount);
  Serial.flush();
  radio.sleep();                                                     // power down LoRa
  esp_sleep_enable_timer_wakeup((uint64_t)g_wakeIntervalSec * 1000000ULL);
...
void handleCommand(const char* type, JsonVariant payload) {
  ...
  // ── Deep-sleep duty cycle ──────────────────────────────────────
  // Awake/sleep durations: stay awake g_awakeSec each cycle, then deep-sleep
  // g_wakeIntervalSec. e.g. awake 5 min, sleep 1 h. Accepts minutes or seconds.
  if (strcmp(type, "sleep_config") == 0 || strcmp(type, "sleep_now") == 0) {
    if (payload.containsKey("enabled"))     g_sleepMode       = payload["enabled"];
    if (strcmp(type, "sleep_now") == 0)     g_sleepMode       = true;
    if (payload.containsKey("sleep_min"))   g_wakeIntervalSec = (uint32_t)constrain((long)(payload["sleep_min"].as<float>() * 60), 10L, 86400L);
    if (payload.containsKey("sleep_sec"))   g_wakeIntervalSec = (uint32_t)constrain((long)payload["sleep_sec"], 10L, 86400L);
    if (payload.containsKey("interval_min"))g_wakeIntervalSec = (uint32_t)constrain((long)payload["interval_min"], 1L, 1440L) * 60UL;  // legacy alias
    if (payload.containsKey("awake_min"))   g_awakeSec        = (uint32_t)constrain((long)(payload["awake_min"].as<float>() * 60), 3L, 86400L);
    if (payload.containsKey("awake_sec"))   g_awakeSec        = (uint32_t)constrain((long)payload["awake_sec"], 3L, 86400L);
    Serial.printf("[SLEEP] config: mode=%d awake=%us sleep=%us\n", g_sleepMode, g_awakeSec, g_wakeIntervalSec);
    sendStatus();
    return;   // the loop deep-sleeps once the awake window elapses

  } else if (strcmp(type, "wake") == 0 || strcmp(type, "sleep_off") == 0) {
    g_sleepMode = false;         // stay awake (normal continuous mode)
    Serial.println("[SLEEP] wake — normal mode");
    sendStatus();
    return;
  }
...
void sendAlive() {
  ...
  doc["valve"]       = (valvePercent > 0) ? "open" : "closed";
...
void sendTelemetry() {
  ...
  // soil_raw kept OFF-air (printed to USB serial only) so the packet stays under
  // the 255-byte LoRa limit.
  Serial.printf("[SOIL] raw=%d\n", analogRead(SOIL_PIN));   // for DRY_VAL/WET_VAL calibration
  JsonDocument doc;
  fillTelemetry(doc, temp, hum);
  sendPacket(doc);
}
